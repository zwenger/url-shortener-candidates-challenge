...
- [x] 10.2 Fixed the pre-existing Dockerfile bug: added `COPY libs/engine/prisma ./libs/engine/prisma` to the `dependencies` stage before `pnpm install --frozen-lockfile`, so the `postinstall: prisma generate` script can find the schema. Verified end-to-end on a fresh volume: `docker compose down -v && docker compose up --build -d` builds through all stages (dependencies → build → production) and the entrypoint applies both migrations (`20260701154709_init`, `20260701170555_add_click_stats`). Functional check: POST shorten → GET `/s/:code` x3 → 302 each time, `clickCount` in the SQLite DB went from 0 to 3 and `lastClickedAt` was set; GET `/s/<nonexistent>` → 404. Restart-survival: `docker compose down` (no `-v`) + `up -d` logs "No pending migrations to apply", the same code still resolves with 302, and `clickCount` persisted and kept incrementing (3 → 4). Cleaned up with `docker compose down -v` and removed the built image.
